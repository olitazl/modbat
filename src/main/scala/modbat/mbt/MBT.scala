...
//    rng = null
  ...
  def configClassLoader(classpath: String) {
    ...
    sourceInfo = new SourceInfo(classLoaderURLs)
  }
  ...
  def maybe (action: Action) = {
    if (or_else) {
      ...
      val choice: Boolean = rng.nextFloat(true) < maybeProbability
  ...
  def maybeBool (pred: () => Boolean) = {
    if (rng.nextFloat(true) < maybeProbability) {
      pred()
    } else {
      false
    }
